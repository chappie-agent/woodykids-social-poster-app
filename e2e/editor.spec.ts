...
async function loadEditor(
  ...
  postId: string,
  cellLabel: string | RegExp,
) {
  // Full page load to /grid seeds the Zustand store from the mocked API
  await page.goto('/grid')
  // Wait until at least one post cell has rendered
  await page.waitForSelector('text=concept', { timeout: 8000 })
  // Click the cell by its accessible name to trigger Next.js client-side navigation
  // (preserves Zustand store state across the route change)
  await page.getByRole('button', { name: cellLabel }).first().click()
  await page.waitForURL(`**/grid/${postId}`, { timeout: 8000 })
}
...
test('editor page title shows product name for shopify post', async ({ page }) => {
  await loadEditor(page, SHOPIFY_POST.id, /Houten treintje set/)
...
test('upload post editor shows caption without MultiPhotoSelector', async ({ page }) => {
  await loadEditor(page, UPLOAD_POST.id, /Eigen upload/)
